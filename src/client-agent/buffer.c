...
int resize_internal_buffer(unsigned int current_capacity, unsigned int desired_capacity) {
    ...
    if (desired_capacity > current_capacity) {
        ...
        if (j < i ) {
            ...
            agent_msg_count, j, i);
            memcpy(new_buffer_ptr, &buffer[j], agent_msg_count);
        } else {
            ...
            memcpy(new_buffer_ptr, &buffer[j], first_part);
            memcpy(new_buffer_ptr + first_part, buffer, i);
        }
