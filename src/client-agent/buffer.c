...
int resize_internal_buffer(unsigned int current_capacity, unsigned int desired_capacity) {
    ...
    if (desired_capacity <= 0) { 
        merror("%s: Invalid new buffer capacity requested: %u.", __func__, desired_capacity);
        return -1;
    }
    ...
    if (desired_capacity > current_capacity) { 
        ...
        if (new_buffer_ptr == NULL) {
            merror("%s: Failed to reallocate client buffer to %u elements. Error: %s", __func__, desired_capacity, strerror(errno));
            w_mutex_unlock(&mutex_lock);
            return -1;
        }
        ...
        // 1. Free memory for messages that will be truncated (if any)
        if (messages_to_discard > 0) {
            ...
            for (unsigned int k = 0; k < messages_to_discard; k++) {
                if (buffer[discard_idx] != NULL) {
                    minfo("DEBUG: Freeing buffer[%u] (ptr: %p)\n", discard_idx, (void*)buffer[discard_idx]);
                    free(buffer[discard_idx]);
                    buffer[discard_idx] = NULL;
                } else {
                    minfo("DEBUG: buffer[%u] is already NULL. Skipping free.\n", discard_idx);
                }
                discard_idx = (discard_idx + 1) % current_capacity;
            }
        }

        // 2. Allocate a new temporary buffer of the desired smaller size
        ...
        // 3. Copy active elements from the old buffer to the new buffer
        ...
        for (unsigned int k = 0; k < new_actual_message_count; k++) {
            temp_buffer[k] = buffer[current_old_idx_for_copy]; // Transfer ownership of pointer
            buffer[current_old_idx_for_copy] = NULL;           // Nullify in old buffer
            current_old_idx_for_copy = (current_old_idx_for_copy + 1) % current_capacity;
        }

        // 4. Free the old buffer and update global pointer
        free(buffer);
        buffer = temp_buffer;

        // 5. Update global buffer state variables for the new smaller buffer
        agent_msg_count = new_actual_message_count;

        // 6. Reset head and tail indices for the new, linear buffer
        j = 0; // Tail is now at the start
        i = agent_msg_count; // Head is after the last copied message
    }
    w_mutex_unlock(&mutex_lock);

    mdebug1("%s: Client buffer resized from %u to %u elements.", __func__, current_capacity, desired_capacity);
    return 0;
}