    ...
    std::shared_ptr<TContext> handleRequest(std::shared_ptr<TContext> data) override
    {
        ...
        else if (originTable == TContext::OriginTable::NetAddress)
        {
            data->m_serializedElement = serializeToJSON(NetElement<TContext>::build(data.get()));
                ...
                {
                    data->m_serializedElement = serializeToJSON(User<TContext>::build(data.get()));
                }
                else if (originTable == TContext::OriginTable::Groups) // New
                {
                    data->m_serializedElement = serializeToJSON(Group<TContext>::build(data.get()));
                }
        else
        {
            logDebug2(LOGGER_DEFAULT_TAG, "UpsertSystemElement::build: not implemented for table %d", static_cast<int>(originTable));
            return nullptr;
        }
